...
const Leadership = () => {
  const leadershipExperts = expertsData.filter(expert => expert.category === 'leadership');
  const [currentExpertIndex, setCurrentExpertIndex] = useState(0);
  const currentExpert = leadershipExperts[currentExpertIndex];

  const handlePrevious = () => {
    setCurrentExpertIndex((prev) => (prev - 1 + leadershipExperts.length) % leadershipExperts.length);
  };

  const handleNext = () => {
    setCurrentExpertIndex((prev) => (prev + 1) % leadershipExperts.length);
  };
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="text-center mb-16">
              ...
              <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
                Experienced leaders driving innovation and growth in sustainable water treatment solutions.
              </p>
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <div className="text-sm text-muted-foreground bg-muted/50 px-4 py-2 rounded-full">
                  {currentExpertIndex + 1} of {leadershipExperts.length}
                </div>
